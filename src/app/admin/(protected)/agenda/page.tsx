...
export default function AdminAgendaPage() {
  ...
  const confirmDelete = async () => {
    ...
    startTransition(async () => {
      const r = await deleteEvento(deleteId, user!.email!);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const r = editing ? await updateEvento(editing.id, form, user!.email!) : await createEvento(form, user!.email!);
  ...
  return (
    <div className="max-w-5xl mx-auto">
            ...
            {eventos.map((ev) => (
              <div key={ev.id} className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => openEdit(ev)} className="p-1.5 rounded-lg hover:bg-gray-200 text-gray-400 hover:text-azul-eletrico transition-colors" aria-label="Editar evento">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleDelete(ev.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" aria-label="Excluir evento">
      ...
      <ConfirmDialog
        ...
        description="Esta ação é irreversível. O evento será excluído permanentemente."
