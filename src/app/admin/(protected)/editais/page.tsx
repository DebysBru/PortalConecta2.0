...
export default function AdminEditaisPage() {
  ...
  const handleTogglePublicacao = (id: string) => {
    ...
    startTransition(async () => {
      const result = await toggleEditalPublicacao(id, user.email!);
      if (result.ok) load();
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    startTransition(async () => {
      const result = await deleteEdital(deleteId, user.email!);
      if (result.ok) load();
      else setError(result.error);
      setDeleting(false);
      setDeleteId(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = editing
        ? await updateEdital(editing.id, form, user!.email!)
  ...
  return (
    <div className="max-w-5xl mx-auto">
              ...
              {editais.map((e) => (
                <div key={e.id} className="p-4 flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-2">
                      ...
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${
                        e.review_status === 'PUBLICADO' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                      }`}>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                    <button
                      onClick={() => handleTogglePublicacao(e.id)}
                      className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-green-600 transition-colors"
                      title={e.review_status === 'PUBLICADO' ? 'Despublicar' : 'Publicar'}
                    >
                      {e.review_status === 'PUBLICADO' ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                    <button onClick={() => openEdit(e)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-azul-eletrico transition-colors" aria-label="Editar edital">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(e.id)} className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" aria-label="Excluir edital">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 justify-end">
                          <button
                            ...
                            title={e.review_status === 'PUBLICADO' ? 'Despublicar' : 'Publicar'}
                          >
                            {e.review_status === 'PUBLICADO' ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                          </button>
                          <button onClick={() => openEdit(e)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-azul-eletrico transition-colors" aria-label="Editar edital">
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDelete(e.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" aria-label="Excluir edital">
      ...
      <ConfirmDialog
        ...
        description="Esta ação é irreversível. O edital e todos os dados associados serão excluídos permanentemente."
