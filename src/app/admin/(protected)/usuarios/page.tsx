...
export default function AdminUsuariosPage() {
  ...
  const handleRoleChange = (userId: string, role: UserRole) => {
    ...
    startTransition(async () => {
      const r = await updateUserRole(userId, role, undefined, currentUser.email!);
  ...
  const confirmDelete = async () => {
    if (!deleteUserId || !currentUser?.email) return;
    setDeleting(true);
    startTransition(async () => {
      const r = await deleteUser(deleteUserId, currentUser.email!);
      if (r.ok) load(); else setError(r.error);
      setDeleting(false);
  ...
  const handleInvite = (e: React.FormEvent) => {
    ...
    if (!currentUser?.email) return;
    startTransition(async () => {
      const r = await inviteUser(inviteEmail, inviteRole, inviteRole === 'PROFESSOR' ? inviteProjetoId : undefined, currentUser.email!);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
              ...
              {users.map((u) => {
                ...
                return (
                  <div key={u.id} className="p-4 flex items-center gap-3">
                    ...
                    {!isSelf && (
                      <button onClick={() => handleDelete(u.id)} className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors flex-shrink-0" aria-label="Excluir usuário">
                  ...
                  {users.map((u) => {
                    ...
                    return (
                      <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {!isSelf && (
                            <button onClick={() => handleDelete(u.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" aria-label="Excluir usuário">
      ...
      <ConfirmDialog
        ...
        description="Esta ação é irreversível. O usuário e todos os dados associados serão excluídos permanentemente."
